...
while True:
    ...
    if (
        ...
        #########################################
        body_index_img = body_index_frame.reshape(
            ((depth_height, depth_width))
        ).astype(np.uint8)
        color_img = color_frame.reshape(
            ((color_height, color_width, 4))
        ).astype(np.uint8)
        depth_img = depth_frame.reshape(((depth_height, depth_width))).astype(
            np.uint16
        )
        infrared_img = infrared_frame.reshape(
            ((depth_height, depth_width))
        ).astype(np.uint16)
        ...
        align_color_img = utils.draw_bodyframe(
            body_frame, kinect, align_color_img
        )  # Overlay body joints on align_color_img
        body_index_img = utils.color_body_index(
            kinect, body_index_img
        )  # Add color to body_index_img

        ######################################
        ### Display 2D images using OpenCV ###
        ######################################
        color_img_resize = cv2.resize(
            color_img, (0, 0), fx=0.5, fy=0.5
        )  # Resize (1080, 1920, 4) into half (540, 960, 4)
        depth_colormap = cv2.applyColorMap(
            cv2.convertScaleAbs(depth_img, alpha=255 / 1500), cv2.COLORMAP_JET
        )  # Scale to display from 0 mm to 1500 mm
        infrared_img = cv2.convertScaleAbs(
            infrared_img, alpha=255 / 65535
        )  # Scale from uint16 to uint8

        cv2.imshow("body index", body_index_img)  # (424, 512)
        cv2.imshow("color", color_img_resize)  # (540, 960, 4)
        cv2.imshow(
            "align color with body joints", align_color_img
        )  # (424, 512)
