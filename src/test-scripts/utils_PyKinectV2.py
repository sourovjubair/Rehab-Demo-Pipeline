# Credits: https://github.com/limgm/PyKinect2
...
def get_align_color_image(
    ...
    colorXYs = np.copy(
        np.ctypeslib.as_array(CSP, shape=(depth_height * depth_width,))
    )  # Convert ctype pointer to array
    colorXYs = colorXYs.view(np.float32).reshape(
        colorXYs.shape + (-1,)
...
def get_single_joint(joints, jointPoints, jointType):
    ...
    if (jointState == PyKinectV2.TrackingState_NotTracked) or (
        jointState == PyKinectV2.TrackingState_Inferred
    ):
...
def get_joint2D(joints, jointPoints):
    joint2D = np.zeros(
        (PyKinectV2.JointType_Count, 2), dtype=np.int32
    )  # [25, 2] Note: Total 25 joints
...
def get_joint3D(joints, jointPoints, depth_img, intrinsics, depth_scale):
    joint3D = np.zeros(
        (PyKinectV2.JointType_Count, 3), dtype=np.float32
    )  # [25, 3] Note: Total 25 joints
...
def get_joint_quaternions(orientations):
    quat = np.zeros(
        (PyKinectV2.JointType_Count, 4), dtype=np.float32
    )  # [25, 4] Note: Total 25 joints
    for i in range(PyKinectV2.JointType_Count):
...
#     align_color_img = align_color_img[
#         :, :, 0:3
#     ]  # Only get the first three channel
...
# def get_single_joint3D_and_orientation(
#     kinect, body_frame, depth_img, intrinsic, depth_scale
# ):
...
#                 )  # Convert joint coordinates to depth space
#                 joint3D = get_joint3D(
#                     joints, joint_points, depth_img, intrinsic, depth_scale
#                 )  # Convert to numpy array format
