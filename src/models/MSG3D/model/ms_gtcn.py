...
class UnfoldTemporalWindows(nn.Module):
    ...
    def forward(self, x):
        ...
        x = (
            x.view(N, C, self.window_size, -1, V)
            .permute(0, 1, 3, 2, 4)
            .contiguous()
        )
...
class SpatialTemporal_MS_GCN(nn.Module):
    def __init__(
        ...
        if disentangled_agg:
            ...
            A_scales = [
                normalize_adjacency_matrix(A) for k in range(num_scales)
            ]
            A_scales = [
                np.linalg.matrix_power(g, k) for k, g in enumerate(A_scales)
            ]
        ...
        if use_Ares:
            self.A_res = nn.init.uniform_(
                nn.Parameter(torch.randn(self.A_scales.shape)), -1e-6, 1e-6
            )
        else:
    ...
    def build_spatial_temporal_graph(self, A_binary, window_size):
        assert isinstance(
            A_binary, np.ndarray
        ), "A_binary should be of type `np.ndarray`"
        V = len(A_binary)
        V_large = V * window_size
        A_binary_with_I = A_binary + np.eye(
            len(A_binary), dtype=A_binary.dtype
        )
    ...
    def forward(self, x):
        ...
        # Build graphs
        A = self.A_scales.to(x.dtype).to(x.device) + self.A_res.to(x.dtype).to(
            x.device
        )
        ...
        agg = (
            agg.permute(0, 3, 1, 2, 4)
            .contiguous()
            .view(N, self.num_scales * C, T, V)
        )
        out = self.mlp(agg)
        out += res
        return self.act(out)
